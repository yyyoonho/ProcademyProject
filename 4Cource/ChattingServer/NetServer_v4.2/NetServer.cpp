...
void CNetServer::PQCS_SendReq(DWORD64 sessionID)
{
	PostQueuedCompletionStatus(_hIOCP, NULL, (ULONG_PTR)sessionID, LPOVERLAPPED(&sendReqToIOCP));
}

bool CNetServer::SendPacketProc(DWORD64 sessionID)
{
	// 세션 검색
	unsigned int idx = GetIdxFromSessionID(sessionID);
	Session* pSession = NULL;
...
void CNetServer::InitPQCS()
{
	sendReqToIOCP.type = SEND;
	sendReqToIOCP.sPacketCount = 0;

	releaseReqToIOCP.type = PQCS;
}

...
void ChatServer::PacketProc_Login(DWORD64 sessionID, SerializePacketPtr pPacket)
{
	...
	{
		RawPtr r;
		newPacket.GetRawPtr(&r);

		r._ptr->SPacketMP.TraceStamp(r._ptr, "Login", __FILE__, __LINE__);
	}
	...
	if (isAlreadyLoggedin == TRUE)
	{
		...
		if (playerArr[oldIdx].state == PLAYER_STATE::DISCONNECTING)
		{
			ReleaseSRWLockExclusive(&playerArr[oldIdx].playerLock);
			return;
		}
		...
		playerArr[oldIdx].state = PLAYER_STATE::DISCONNECTING;
...
void ChatServer::PacketProc_Message(DWORD64 sessionID, SerializePacketPtr pPacket)
{
	...
	// TRACE
	{
		...
		r._ptr->SPacketMP.TraceStamp(r._ptr, "Message", __FILE__, __LINE__);
	}

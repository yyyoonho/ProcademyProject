...
UINT FuncA(LPVOID lpThreadParameter)
{
    ...
    {
        g_flag[0] = true;   // store g_flag[0]
        g_turn = 0;         // store g_turn

        while (1)
        {
            cap_A = g_flag[1];  // load g_flag[1]
            cap_B = g_turn;     // load g_turn
...
UINT FuncB(LPVOID lpThreadParameter)
{
    ...
    while (count++ < 10000000)
    {
        g_flag[1] = true;   // store g_flag[1]
        g_turn = 1;         // store g_turn

        while (1)
        {
            cap_C = g_flag[0];  // load g_flag[0]
            cap_D = g_turn;     // load g_turn
